...
export class NodeEditorComponent implements AfterViewInit {
  ...
  nodes: any[] = [
    { id: 0, reflexive: false, label: '0' }/*
    { id: 1, reflexive: true, label: '1'},
    { id: 2, reflexive: false, label: '2'}*/
  ];
  links: any[] = [
  ...
  searchLevel: number = 3;

  includeNodes(includes, text, level) {
    if (level <= this.searchLevel) {
      includes.push(text);
      let wordRelationship = this.centrality.relationships.get(text);
      for (let link of wordRelationship.relationships) {
        if (!includes.includes(link)) {
          this.includeNodes(includes, link, level+1);
        }
      }
    }
  }
  ...
  search() {
    ...
    this.includeNodes(includes, this.searchText, 0);

    let nodeIndex = 0;
    let nodeIndices = [];

    for (let node of this.nodes) {
      if (!includes.includes(node.label)) {
        ...
        for (let linkIndex of linkIndices.reverse()) {
    ...
    for (let nodeIndex of nodeIndices.reverse()) {
    ...
    this.analysisService.analysis.subscribe((value) => {
      ...
      if (value && value['complete_document']) {
        ...
        for (let note of notes) {
          ...
          this.centrality.addWordPair(a, b);
          //this.analysis += "" + a + "-" + b;
        }
        let wordId = 1;
        ...
        for (let word of this.centrality.relationships.keys()) {
          console.log('word is ' + word);
          //let wordRelationship = centrality.relationships.get(word);
          this.nodes.push({ id: wordId, reflexive: true, label: word });
          wordIds[word] = wordId;
        ...
        let linkId = 1;
        for (let word of this.centrality.relationships.keys()) {
          let wordId = wordIds[word];
          let wordRelationship = this.centrality.relationships.get(word);
          for (let link of wordRelationship.relationships) {
            let targetId = wordIds[link];
              this.links.push({source: this.nodes[wordId] , target: this.nodes[targetId], left:false, right: false, label: "" + linkId });
              linkId += 1;
  ...
  restart() {
    ...
    this.circle.selectAll('circle')
      ...
      //this.circle.selectAll('circle').text((d) => d.label);
    // remove old nodes
    ...
    g.append('svg:circle')
      ...
      .on('mousedown', (d) => {
        console.log("mouse down on a node");
        if (d3.event.ctrlKey) return;
  ...
  keydown = () => {
    //if(this.graphContainer.nativeElement.offsetParent === null) return;
    console.log("Keycode is " + d3.event.keyCode);
